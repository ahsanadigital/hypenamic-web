...
<header class="py-5">
    <div class="container-fluid">
        <div class="d-md-flex justify-content-between">
            <div class="left">
                <h3>Edit Data</h3>
                <p class="text-muted m-0">Mengubah data kegiatan "{{ $event->event_name }}".</p>
            </div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb px-sm-0 px-md-3 bg-transparent">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Edit Data {{ $event->event_name }}</li>
                </ol>
            </nav>
        </div>
    </div>
...
<section id="main-content">
    <div class="container">
        <div class="card card-body">
            ...
            <form action="{{ route('admin.event-editProcess') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-8 mb-3 mb-md-0">
                        ...
                        <div class="form-group floating-alt">
                            <input type="text" class="form-control floating" name="event_name" value="{{ $event->event_name }}" id="event_name" placeholder="Masukkan nama kegiatan" required />
                            <label for="event_name">Nama Kegiatan</label>
                        </div>

                        <div class="form-group" onclick="openModal('#userModal')">
                            <input type="hidden" name="id_user" value="{{ $author->username }}" />
                            <label for="id_user">Nama Penyelenggara</label>
                            <input type="text" class="form-control form-button floating" readonly id="id_user" value="{{ $author->fullname }}" placeholder="Masukkan penyelenggara" required />
                        </div>
                        ...
                        <div class="form-group" onclick="openModal('#categoryModal')">
                            ...
                            <input type="text" class="form-control form-button floating" value="{{ $event->getCategory()->first()->cat_name }}" readonly id="category" placeholder="Masukkan kategori" required />
                        </div>

                        <div class="form-group">
                            <label for="event_desc">Deskripsi Kegiatan</label>
                            <textarea id="event_desc" placeholder="Masukkan deskripsi kegiatanmu" class="form-control summernote" rows="3" name="event_desc">{{ $event->event_desc }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="event_desc_short">Deskripsi Event</label>
                            <textarea name="event_desc_short" id="event_desc_short" cols="30" rows="5" class="form-control">{{ $event->event_desc_short }}</textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <div class="input-group-prepend input-group-append">
                                    <div class="input-group-text"><i class="fas fa-chevron-right"></i></div>
                                </div>
                        ...
                        <div class="form-group">
                            <div class="heading d-flex">
                                <div class="left">
                                    ...
                                    <p class="text-muted">Tambahkan syarat dan ketentuan untuk memberikan penjelasan terkait aturan kegiatanmu</p>
                                </div>
                                <div class="right"><button class="btn btn-light" type="button" data-toggle="modal" data-target="#tosModal"><i class="fas fa-plus"></i></button></div>
                            </div>

                            <div class="result-tos accordion" id="tos-collapse">
                                @if($event->event_tos)
                                    @foreach (json_decode($event->event_tos) as $index => $tos)
                                    <div class="card" id="{{ md5($tos->title) }}">

                                        <input type="hidden" value="{{ $tos->title }}" name="tos[{{ md5($tos->title) }}][title]" />
                                        <input type="hidden" value="{{ $tos->desc }}" name="tos[{{ md5($tos->title) }}][desc]" />

                                        <div class="card-header" id="heading-{{ md5($tos->title) }}">
                                            <div class="d-flex justify-content-between">
                                                <div class="left" style="width:85%">
                                                    <button class="btn btn-link btn-block text-left" type="button" data-toggle="collapse" data-target="#collapse-{{ md5($tos->title) }}" aria-expanded="true" aria-controls="collapse-{{ md5($tos->title) }}">{{ $tos->title }}</button>
                                                </div>
                                                <div class="right">
                                                    <button type="button" class="btn btn-danger btn-delete-tos" data-target="#{{ md5($tos->title) }}"><i class="fas fa-trash"></i></button>
                                                </div>
                                            </div>
                                        </div>

                                        <div id="collapse-{{ md5($tos->title) }}" class="collapse" aria-labelledby="heading-{{ md5($tos->title) }}" data-parent="#tos-collapse">
                                            <div class="card-body">
                                                {!! $tos->desc !!}
                                            </div>
                                        </div>
                    ...
                    <div class="col-md-4 mb-3 mb-md-0">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="input-card-container">
                                <label for="event_do" class="input-custom-card mb-3 mb-md-0">
                                    <input type="radio" class="input-custom"{{ $event->event_do == 'offline' ? ' checked' : '' }} name="event_do" value="offline" id="event_do" />
                                ...
                                <label for="event_do2" class="input-custom-card">
                                    <input type="radio" class="input-custom"{{ $event->event_do == 'online' ? ' checked' : '' }} name="event_do" value="online" id="event_do2" />
                                    <div class="input-wrap">
                                        <div class="input-header">
                                            <div class="indicator"></div>
                                            <span>Online</span>
                        ...
                        <div class="collapse{{ $event->event_do == 'online' ? ' show' : '' }}" id="online">
                            <div class="form-group floating">
                                <input type="text" value="{{ $event->url }}" class="form-control" id="url" name="url" placeholder="Masukkan URL Zoom atau kegiatan" />
                                <label for="url">Tautan Kegiatan</label>
                            </div>
                        </div>

                        {{-- @dd($event->event_do) --}}
                        <div class="collapse{{ $event->event_do == 'offline' ? ' show' : '' }}" id="offline">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="alamat" id="alamat" class="form-control" placeholder="Masukkan alamat kegiatan" rows="3">{{ $event->alamat }}</textarea>
                            </div>

                        </div>

                        <div class="form-group">
                            <img src="{{ $event->event_banner ? asset($event->event_banner) : asset('assets/images/icon-empty.png') }}" class="w-100 rounded mb-3" />
                        ...
                        <div class="form-group">
                            <img src="{{ $event->thumbnail ? asset($event->thumbnail) : asset('assets/images/icon-empty.png') }}" class="w-100 rounded mb-3" />
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="input-card-container">
                                <label for="open" class="input-custom-card">
                                    <input type="radio"{{ $event->status == 'open' ? ' checked' : '' }} class="input-custom" name="status" value="open" id="open" />
                                    <div class="input-wrap">
                                        <div class="input-header">
                                            <div class="indicator"></div>
                                ...
                                <label for="closed" class="input-custom-card">
                                    <input type="radio"{{ $event->status == 'closed' ? ' checked' : '' }} class="input-custom" name="status" value="closed" id="closed" />
                                    <div class="input-wrap">
                                        <div class="input-header">
                                            <div class="indicator"></div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="input-card-container">
                                <label for="free" class="input-custom-card">
                                    <input type="radio"{{ $event->event_type == 'free' ? ' checked' : '' }} class="input-custom" name="event_type" value="free" id="free" />
                                    <div class="input-wrap">
                                        <div class="input-header">
                                            <div class="indicator"></div>
                                            <span>Gratis</span>
                                ...
                                <label for="bayar" class="input-custom-card">
                                    <input type="radio"{{ $event->event_type == 'bayar' ? ' checked' : '' }} class="input-custom" name="event_type" value="bayar" id="bayar" />
                                    <div class="input-wrap">
                                        <div class="input-header">
                                            <div class="indicator"></div>
                                            <span>Berbayar</span>
...
$('#start_date').datetimepicker({
    ...
    showClose: true,
    showClear: true,
    minDate: moment('{{ $event->start_date }} {{ $event->start_time }}').format('YYYY-MM-DD HH:mm:ss'),
    icons: {
        time: 'fas fa-clock',
    ...
    defaultDate: moment('{{ $event->start_date }} {{ $event->start_time }}').format('YYYY-MM-DD HH:mm:ss'),
});
$('#end_date').datetimepicker({
    ...
    defaultDate: moment('{{ $event->end_date }} {{ $event->end_time }}').format('YYYY-MM-DD HH:mm:ss'),
});
...
axios.get('{{ route("wilayah.provinsi") }}').then((e) => {
    ...
    let html = '';
    $.each(data, (a, b) => {
        if(b.id == '{{ $event->provinsi }}') {
            html += `<option value="${b.id}" selected>${b.nama}</option>`;
        } else {
            html += `<option value="${b.id}">${b.nama}</option>`;
        }
    });
    $('select#provinsi').html(html);
});

@if ($event->kecamatan !== null OR $event->kelurahan !== null OR $event->kabupaten !== null OR $event->provinsi !== null)
    @php
    ...
    @endphp
    $('select#kabupaten').append(`<option value="{{ $kabupaten['id'] }}">{{ $kabupaten['nama'] }}</option>`);
    $('select#kecamatan').append(`<option value="{{ $kecamatan['id'] }}">{{ $kecamatan['nama'] }}</option>`);
    $('select#kelurahan').append(`<option value="{{ $kelurahan['id'] }}">{{ $kelurahan['nama'] }}</option>`);
@endif
    ...
    $.ajax({
        ...
        success(result) {
            ...
            successAlert.fire({
                ...
                title: 'Berhasil!',
                text: 'Berhasil mengubah data, sistem akan mengarahkan anda ke halaman detail eevnt ini.',
                didOpen() {
                    Swal.showLoading()
                }
            });
