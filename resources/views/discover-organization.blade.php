    ...
    <div class="container">
        ...
        <nav>
            <ul class="px-0 mb-0 breadcrumb bg-transparent">
                ...
                <li class="breadcrumb-item"><a href="{{ route('jelajah') }}" class="breadcrumb-link">Semua Event</a></li>
                <li class="breadcrumb-item active"><span class="active breadcrumb-link">Organisasi "{{ $query->institution }}"</span></li>
            </ul>
        </nav>
        <div class="d-md-flex justify-content-between mb-3">
            <div class="left">
                <h1>Organisasi "{{ $query->institution }}"</h1>
                <ul class="list-unstyled">
                    @php
                    $provinsi = $wilayah->getProvinsi($query->provinsi)['nama'];
                    $kabupaten = $wilayah->getKabupaten($query->provinsi, $query->kabupaten)['nama'];
                    $kecamatan = $wilayah->getKecamatan($query->kabupaten, $query->kecamatan)['nama'];
                    $kelurahan = $wilayah->getKelurahan($query->kecamatan, $query->kelurahan)['nama'];
                    @endphp
                    <li><i class="fas fa-map-marker fa-fw mr-2"></i>{{ $query->alamat }}, {{ $kelurahan }}, {{ $kecamatan }}, {{ $kabupaten }}, {{ $provinsi }}</li>
                    <li><i class="fas fa-clock fa-fw mr-2"></i>Bergabung Sejak {{ $query->created_at->isoFormat('LLLL') }}</li>
                </ul>
            </div>
        ...
        @if ($category->count() > 0)
            ...
            <a href="{{ route('jelajah.kategori', $data->slug) }}" class="{{ $query->slug == $data->slug ? 'active ' : '' }}category-lists">{{ $data->cat_name }}</a>
            @endforeach
        @else
